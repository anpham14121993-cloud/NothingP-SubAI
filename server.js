                            // NothingP AIOsubtitles v3.9.77 — 20K/120 + 5 in-flight/key + 15 RPM/key + fast timeout fallback + Gate Cross-Instance Fix
                            ...
                            <body>
                            <div class="container">
                            <h2>NothingP AIOsubtitles v3.9.77</h2>
                            <form id="configForm">
                            ...
                            app.get('/healthz', (req, res) => {
                              res.status(200).json({ ok: true, version: '3.9.77', uptime: Math.round(process.uptime()) });
                            });
                            ...
                            // ============================================================
                            // v3.9.77 SDH CHARACTER-GUIDE EVIDENCE
                            // SDH is NEVER required for translation. It is an optional evidence layer.
                            // ============================================================
                            
            // v3.9.77 AGE / KINSHIP / HIERARCHY EVIDENCE
    ...
    function normalizeAgeNumber(value) {
            ...
            function scoreSdhCandidate(meta = {}, releaseName = '') {
                            ...
                            QUY TẮC:
                            - Chỉ dùng movieContext, subtitleSample và SDH evidence của chính phim/tập.
    ...
    - Không đoán giới tính chỉ từ tên. Nếu không đủ bằng chứng thì gender="unknown".
                            ...
                            app.get(['/translate-sub', '/translate-sub/g/:gate'], async (req, res) => {
                              ...
                              try {
                                ...
                                const backgroundJob = (async () => {
                                  ...
                                  // Character gender evidence from metadata already fetched for this title.
                              // Different Cinemeta/TMDB-compatible payloads expose cast in different shapes,
                              // therefore collect defensively and silently fall back when unavailable.
                              try {
                                const possibleMetaObjects = [];
                                if (typeof meta !== 'undefined' && meta) possibleMetaObjects.push(meta);
                                if (typeof metaData !== 'undefined' && metaData) possibleMetaObjects.push(metaData);
                                if (typeof cinemetaData !== 'undefined' && cinemetaData) possibleMetaObjects.push(cinemetaData);

                                let rawCast = [];
                                for (const obj of possibleMetaObjects) {
                                  const candidates = [
                                    obj?.cast, obj?.credits?.cast, obj?.meta?.cast, obj?.meta?.credits?.cast
                                  ];
                                  const found = candidates.find(Array.isArray);
                                  if (found?.length) { rawCast = found; break; }
                                }
                    ...
                    castAgeEvidence = formatCastAgeEvidence(ageItems, 5000);
                                ...
                                console.warn('[Character Gender] metadata extraction skipped:', e.message);
                                castGenderEvidence = '';
                              }
                              ...
                              const subtitleSample = buildSubtitleContextSample(originalSrt, 9000);
                                  ...
                                  const protectedTerms = extractProtectedTermsFromGuide(characterGuide);
