                                // NothingP AIOsubtitles v3.9.78 — 20K/120 + 5 in-flight/key + 15 RPM/key + fast timeout fallback + Gate Cross-Instance Fix
                                ...
                                <body>
                                <div class="container">
                                <h2>NothingP AIOsubtitles v3.9.78</h2>
                                <form id="configForm">
                                ...
                                app.get('/healthz', (req, res) => {
                                  res.status(200).json({ ok: true, version: '3.9.78', uptime: Math.round(process.uptime()) });
                                });
                                ...
                                // ============================================================
                                // v3.9.78 SDH CHARACTER-GUIDE EVIDENCE
                                // SDH is NEVER required for translation. It is an optional evidence layer.
                                // ============================================================
                                
                // v3.9.78 AGE / KINSHIP / HIERARCHY EVIDENCE
                
        function extractProtectedTermsFromGuide(guideText = '') {
          const protectedTerms = [];
          const seen = new Set();
          try {
            let raw = String(guideText || '').trim()
              .replace(/^```(?:json)?\s*/i, '')
              .replace(/\s*```$/i, '');
            ...
            // Character names and aliases are always protected.
            for (const c of Array.isArray(guide?.characters) ? guide.characters : []) {
              add(c?.name);
              for (const alias of Array.isArray(c?.aliases) ? c.aliases : []) add(alias);
            }

            // Explicit non-translatable terms are protected.
            for (const t of Array.isArray(guide?.terms) ? guide.terms : []) {
              if (t?.translate === false || [
                'proper_name','character','place','organization','creature',
                'artifact','spell','fictional_term'
              ].includes(String(t?.type || '').toLowerCase())) {
                add(t?.source);
              }
            }
          } catch (e) {
            console.warn('[Protected Terms] Guide JSON parse skipped:', e.message);
          }
          return protectedTerms;
        }

        function formatProtectedTermsForPrompt(terms = [], maxChars = 6000) {
    ...
    function normalizeGenderValue(value) {
      ...
      if (!v) return 'unknown';
      if (['1', 'female', 'f', 'woman', 'girl'].includes(v)) return 'female';
      if (['2', 'male', 'm', 'man', 'boy'].includes(v)) return 'male';
      return 'unknown';
    }

    function buildCastGenderEvidence(rawCast = []) {
      if (!Array.isArray(rawCast)) return [];
      const out = [];
      const seen = new Set();
      for (const item of rawCast) {
        const character = String(item?.character || item?.role || item?.character_name || item?.characterName || '').trim();
        if (!character) continue;
        const actor = String(item?.name || item?.actor || item?.person_name || item?.personName || '').trim();

        const characterGenderRaw =
          item?.character_gender ?? item?.characterGender ?? item?.role_gender ?? item?.roleGender;
        const actorGenderRaw =
          item?.gender ?? item?.person_gender ?? item?.personGender;

        let gender = normalizeGenderValue(characterGenderRaw);
        let genderSource = gender !== 'unknown' ? 'character_metadata' : '';
        if (gender === 'unknown') {
          gender = normalizeGenderValue(actorGenderRaw);
          if (gender !== 'unknown') genderSource = 'cast_person_metadata';
        }
        if (gender === 'unknown') continue;

        const key = `${character.toLowerCase()}|${gender}`;
        if (seen.has(key)) continue;
        seen.add(key);
        out.push({
          character, actor, gender, genderSource,
          confidence: genderSource === 'character_metadata' ? 'high' : 'medium'
        });
      }
      return out;
    }

    function formatCastGenderEvidence(items = [], maxChars = 6000) {
      if (!Array.isArray(items) || !items.length) return '';
      return items.map(x =>
        `${x.character} | gender=${x.gender} | actor=${x.actor || 'unknown'} | source=${x.genderSource} | confidence=${x.confidence}`
      ).join('\n').slice(0, maxChars);
    }
    ...
    function normalizeAgeNumber(value) {
                ...
                function buildCastAgeEvidence(rawCast = []) {
                  if (!Array.isArray(rawCast)) return [];
                  ...
                  for (const item of rawCast) {
                    ...
                    if (!character) continue;
                    const actor = String(item?.name || item?.actor || item?.person_name || item?.personName || '').trim();
                    const characterAge = normalizeAgeNumber(item?.character_age ?? item?.characterAge ?? item?.role_age ?? item?.roleAge);
                    const actorAge = normalizeAgeNumber(item?.age ?? item?.actor_age ?? item?.actorAge ?? item?.person_age ?? item?.personAge);
                    let age = null, ageGroup = 'unknown', ageSource = '', confidence = 'low';
                    if (characterAge != null) {
                      age = characterAge; ageGroup = ageGroupFromAge(characterAge);
                      ageSource = 'character_metadata'; confidence = 'high';
                    } else if (actorAge != null) {
                      ageGroup = ageGroupFromAge(actorAge);
                      ageSource = 'cast_person_metadata'; confidence = 'low';
                    }
                    if (ageGroup === 'unknown') continue;
                    const key = `${character.toLowerCase()}|${ageGroup}`;
                    if (seen.has(key)) continue;
                    seen.add(key);
                    out.push({ character, actor, age, ageGroup, ageSource, confidence });
                  }
                  return out;
                }
                function formatCastAgeEvidence(items = [], maxChars = 5000) {
                  if (!Array.isArray(items) || !items.length) return '';
                  return items.map(x => `${x.character} | age=${x.age ?? 'unknown'} | age_group=${x.ageGroup} | actor=${x.actor || 'unknown'} | source=${x.ageSource} | confidence=${x.confidence}`)
                    .join('\n').slice(0, maxChars);
                }

                function scoreSdhCandidate(meta = {}, releaseName = '') {
                                  const text = [
                                    releaseName, meta?.releaseName, meta?.release_name, meta?.name,
                                    meta?.title, meta?.fileName, meta?.file_name, meta?.productionType
                                  ].filter(Boolean).join(' ');
                                  let score = 0;
                                  const explicit = [
                                    meta?.hearing_impaired, meta?.hearingImpaired, meta?.hearingimpaired,
                                    meta?.is_sdh, meta?.isSDH, meta?.sdh, meta?.hi
                                  ];
                                  if (explicit.some(v => v === true || v === 1 || String(v).toLowerCase() === 'true')) score += 100;
                                  if (/\bSDH\b/i.test(text)) score += 70;
                                  if (/\bhearing[\s._-]*impaired\b/i.test(text)) score += 70;
                                  if (/(?:^|[\s._\-[\(])HI(?:$|[\s._\-\]\)])/i.test(text)) score += 45;
                                  if (/(?:^|[\s._\-[\(])CC(?:$|[\s._\-\]\)])/i.test(text)) score += 25;
                                  return score;
                                }

                                function buildSdhEvidenceSample(srt, maxChars = 9000) {
                                  const cues = parseSrtCues(String(srt || ''));
                                  const out = [];
                                  for (const cue of cues) {
                                    const body = String(cue.text || '').trim();
                                    if (!body) continue;
                                    const bracket = body.match(/^\[([^\]\n]{1,60})\]\s*(.*)$/s);
                                    const colon = body.match(/^([A-Z][A-Z0-9 ._'’-]{1,35}):\s*(.*)$/s);
                                    if (bracket) {
                                      const label = bracket[1].trim();
                                      const rest = bracket[2].trim();
                                      const sound = /^(music|door|phone|bell|footsteps|applause|laughing|crying|sighs?|gasps?|groans?|screams?|thunder|wind|engine|gunshots?|knocking|ringing|beeping)\b/i.test(label);
                                      out.push(sound ? `[SDH CONTEXT] ${label}${rest ? ': ' + rest : ''}` :
                                                       `[SPEAKER TAG] ${label}${rest ? ': ' + rest : ''}`);
                                    } else if (colon) {
                                      out.push(`[SPEAKER TAG] ${colon[1].trim()}: ${colon[2].trim()}`);
                                    } else if (/^\([^)\n]{1,60}\)/.test(body) || /♪/.test(body)) {
                                      out.push(`[SDH CONTEXT] ${body}`);
                                    }
                                ...
                                function stripSdhMarkupFromCueText(text) {
                                  let s = String(text || '');
                                  s = s.replace(/^\s*\[[^\]\n]{1,60}\]\s*(?=\S)/, '');
                                  s = s.replace(/^\s*[A-Z][A-Z0-9 ._'’-]{1,35}:\s*(?=\S)/, '');
                                  if (/^\s*\[[^\]\n]{1,100}\]\s*$/.test(s)) return '';
                                  if (/^\s*\([^)\n]{1,100}\)\s*$/.test(s)) return '';
                                  if (/^\s*♪[\s\S]*$/m.test(s)) return '';
                                  return s.trim();
                                }

                                function cleanSelectedSdhForTranslation(srt) {
                                  const cues = parseSrtCues(String(srt || ''));
                                  if (!cues.length) return String(srt || '');
                                  return buildSrtFromCues(cues.map(c => ({ ...c, text: stripSdhMarkupFromCueText(c.text) })));
                                }
                                ...
                                QUY TẮC:
                                - Chỉ dùng movieContext, subtitleSample, SDH evidence và CAST evidence của chính phim/tập.
                                - CAST/GENDER evidence chỉ hỗ trợ character identity + gender; không suy ra quan hệ/tình cảm.
                                - CAST/AGE evidence: tuổi nhân vật từ character metadata mạnh hơn; tuổi diễn viên chỉ là bằng chứng phụ cho age_group, không phải tuổi chính xác của nhân vật.
                                - SDH speaker tag như [John], JOHN: là bằng chứng mạnh để xác định NGƯỜI NÓI; sound/action tag không phải tên nhân vật.
                                - TÊN RIÊNG / THUẬT NGỮ HƯ CẤU: nhận diện tên nhân vật, địa danh, tổ chức, sinh vật/thực thể, vật phẩm, phép thuật và tên/thuật ngữ hư cấu có bằng chứng trong metadata/SDH/phụ đề.
        - Với tên cần giữ nguyên, ghi vào terms với source=target giống hệt nhau và translate=false.
        - Ví dụ: Malivore phải là {"source":"Malivore","target":"Malivore","type":"proper_name","translate":false}; KHÔNG dịch nghĩa thành một danh từ tiếng Việt.
        - Không khóa một từ chỉ vì nó viết hoa đầu câu. Chỉ đánh dấu protected khi có bằng chứng đó là tên riêng/thuật ngữ cần giữ nguyên.
        - Không đoán giới tính chỉ từ tên. Nếu không đủ bằng chứng thì gender="unknown".
                                - Không bịa tuổi, giới tính, quan hệ, vai vế, tình cảm hay tình tiết.
                                - Xưng hô phải theo HƯỚNG. self = cách người nói tự xưng; address = cách người nói gọi người nghe.
                                - A->B và B->A là hai hướng độc lập. TUYỆT ĐỐI không đảo self/address.
                                - Giới tính chỉ hỗ trợ nhận diện; không mặc định nam+nữ luôn là anh/em.
                        - Khi chọn xưng hô tiếng Việt phải xét đồng thời gender + relative_age + kinship + hierarchy + relationship + register.
                        - Quan hệ rõ như grandfather/granddaughter, grandmother/grandson, uncle/niece, aunt/nephew, parent/child có ưu tiên cao hơn suy đoán tuổi.
                        - Nếu A lớn hơn B rất rõ và quan hệ phù hợp, tránh tự động dùng anh/em hoặc cậu/tớ; chọn ông/bà/bác/chú/cô/cháu khi evidence hỗ trợ.
                        - Không dùng ông/bà/bác/chú/cô/cháu chỉ vì một nhân vật trông hoặc được suy đoán là lớn tuổi; phải có evidence tuổi tương đối/quan hệ/vai vế.
                                ...
                                JSON BẮT BUỘC:
                                {
                                  "characters": [{"name":"","aliases":[],"presence":"appears|mentioned","gender":"male|female|unknown","gender_source":"character_metadata|cast_person_metadata|dialogue|sdh|unknown","age":null,"age_group":"child|teen|young_adult|adult|middle_aged|elderly|unknown","age_source":"character_metadata|cast_person_metadata|dialogue|sdh|unknown","actor":"","role":"","voice":""}],
                                  "relationships": [{"a":"","b":"","relation":"","status":"","a_gender":"male|female|unknown","b_gender":"male|female|unknown","relative_age":"a_much_older|a_older|similar|b_older|b_much_older|unknown","kinship":"","hierarchy":"","a_to_b":{"self":"","address":""},"b_to_a":{"self":"","address":""},"pronoun_reason":"","confidence":"high|medium|low"}],
                                  "changes": [{"a":"","b":"","event":"","from":"","to":"","pronoun_effect":""}],
                                  "terms": [{"source":"","target":"","type":"proper_name|character|place|organization|creature|artifact|spell|fictional_term|translatable_term","translate":true}],
                                ...
                                SDH EVIDENCE (chỉ làm bằng chứng speaker/context, không phải nội dung cần dịch):
                                ${sdhSample || 'Không có SDH evidence.'}

                                CAST/GENDER EVIDENCE:
                                ${castGenderEvidence || 'Không có structured cast/gender evidence.'}

                                CAST/AGE EVIDENCE:
                                ${castAgeEvidence || 'Không có structured cast/age evidence.'}`;
                                ...
                                async function handleSubtitles(req, res, encodedConfig) {
                                  ...
                                  const fetchOpenSubtitles = async () => {
                                    ...
                                    try {
                                      ...
                                      const osMapped = await Promise.allSettled(osSelected.map(async item => {
                                        ...
                                        const sdhScore = scoreSdhCandidate(item.attributes || {}, releaseName);
                                        if (sdhScore > 0) sdhCandidates.push({ provider: 'os', fileId: String(file.file_id), releaseName, score: sdhScore });
                                    ...
                                    try {
                                      ...
                                      for (const { sub, vi } of mapped) {
                                        ...
                                        if (vi && isVietnamese(lang)) {
                                          ...
                                          const sdhScore = scoreSdhCandidate(sub, releaseName);
                                          if (sdhScore > 0) sdhCandidates.push({ provider: 'subdl', sourceUrl: dlUrl, releaseName, score: sdhScore });
                                  ...
                                  const fetchSubSource = async () => {
                                    ...
                                    try {
                                      ...
                                      const episodeMatchesSubSource = sub => {
                                        ...
                                        // which made this filter silently PASS wrong-episode releases (fail-open).
                                        // Reuse subtitleName()'s array/object-aware extraction instead, and scan
                                        // EVERY SxxExx token found (a release can bundle several files/episodes),
                                        // accepting if any token matches the requested episode.
                                        const names = [
                                          subtitleName(sub, ''),
                                      ...
                                      for (const item of selectedSubs) {
                                        ...
                                        if (isViSelected && isVietnamese(subLanguage)) {
                                          ...
                                          const sdhScore = scoreSdhCandidate(sub, releaseName);
                                          if (sdhScore > 0) sdhCandidates.push({ provider: 'subsource', subtitleId: String(sub.subtitleId), releaseName, score: sdhScore });
                                          const aiUrl =
                                  ...
                                  sdhCandidates.sort((a, b) => (b.score || 0) - (a.score || 0));
                                  const bestSdh = sdhCandidates[0] || null;
                                  if (bestSdh) {
                                    englishOriginalSubtitles = englishOriginalSubtitles.map(sub => {
                                      try {
                                        const u = new URL(sub.url);
                                        u.searchParams.set('guideProvider', bestSdh.provider);
                                        if (bestSdh.fileId) u.searchParams.set('guideFileId', bestSdh.fileId);
                                        if (bestSdh.sourceUrl) u.searchParams.set('guideSourceUrl', bestSdh.sourceUrl);
                                        if (bestSdh.subtitleId) u.searchParams.set('guideSubtitleId', bestSdh.subtitleId);
                                        u.searchParams.set('guideSdh', '1');
                                        return { ...sub, url: u.toString() };
                                      } catch { return sub; }
                                    });
                                    console.log(`[SDH Guide] selected ${bestSdh.provider} | score=${bestSdh.score} | ${bestSdh.releaseName || ''}`);
                                  } else {
                                    console.log('[SDH Guide] no SDH candidate; normal guide fallback.');
                                  }
                                  ...
                                  // Deduplicate ONLY the same underlying subtitle from the same provider.
                                  // Do NOT deduplicate by displayed release name: different providers/files can
                                  // legitimately use the same release name and must remain selectable.
                                  const seenSubtitleKeys = new Set();
                                  const subtitleDedupKey = sub => {
                                    const url = String(sub?.url || '').trim();
                                    const lowerUrl = url.toLowerCase();

                                    if (lowerUrl.includes('/proxy-os?')) {
                                      try {
                                        const u = new URL(url);
                                        const fileId = String(u.searchParams.get('fileId') || '').trim();
                                        if (fileId) return `os:file:${fileId}`;
                                      } catch (_) {}
                                      return `os:url:${url}`;
                                    }

                                    if (lowerUrl.includes('/proxy-subdl?')) {
                                      try {
                                        const u = new URL(url);
                                        const raw = u.searchParams.get('url') || '';
                                        if (raw) return `subdl:url:${raw}`;
                                      } catch (_) {}
                                      return `subdl:url:${url}`;
                                    }

                                    if (lowerUrl.includes('/subsource-sub/')) {
                                      try {
                                        const u = new URL(url);
                                        const pathMatch = u.pathname.match(/\/subsource-sub\/([^/?]+)/i);
                                        if (pathMatch?.[1]) return `subsource:id:${decodeURIComponent(pathMatch[1])}`;
                                      } catch (_) {}
                                      return `subsource:url:${url}`;
                                    }

                                    if (lowerUrl.includes('/translate-sub?')) {
                                      try {
                                        const u = new URL(url);
                                        const provider = String(u.searchParams.get('provider') || '').toLowerCase();
                                        const fileId = String(u.searchParams.get('fileId') || '').trim();
                                        const subtitleId = String(u.searchParams.get('subtitleId') || '').trim();
                                        const sourceUrl = String(u.searchParams.get('sourceUrl') || '').trim();
                                        if (provider === 'os' && fileId) return `os:file:${fileId}`;
                                        if (provider === 'subdl' && sourceUrl) return `subdl:url:${sourceUrl}`;
                                        if (provider === 'subsource' && subtitleId) return `subsource:id:${subtitleId}`;
                                      } catch (_) {}
                                    }
                                  ...
                                  const subtitles = [...nativeVietSubtitles, ...englishOriginalSubtitles].filter(sub => {
                                    const key = subtitleDedupKey(sub);
                                ...
                                app.get(['/translate-sub', '/translate-sub/g/:gate'], async (req, res) => {
                                  const { url, provider, fileId, sourceUrl, subtitleId, model, config: configQuery, imdbId, type, season, episode, source, target, gate: gateQuery,
                                    guideProvider, guideFileId, guideSourceUrl, guideSubtitleId, guideSdh } = req.query;
                                  ...
                                  try {
                                    ...
                                    const backgroundJob = (async () => {
                                      ...
                                      let sdhEvidenceSrt = '';
                                      if (String(guideSdh || '') === '1' && guideProvider) {
                                        try {
                                          if (guideProvider === 'os' && guideFileId) {
                                            const apiKeyOS = sourceConfig.opensubtitlesKey || '2015';
                                            const d = await axios.post('https://api.opensubtitles.com/api/v1/download',
                                              { file_id: String(guideFileId) },
                                              { headers: { 'Api-Key': apiKeyOS, ...API_HEADERS, 'Content-Type': 'application/json' }, timeout: 10000 });
                                            if (d.data?.link) sdhEvidenceSrt = await fetchSubtitleText(d.data.link, SUBTITLE_BROWSER_HEADERS, 25000, 1);
                                          } else if (guideProvider === 'subdl' && guideSourceUrl) {
                                            const key = String(sourceConfig.subdlKey || '');
                                            sdhEvidenceSrt = await fetchSubtitleText(guideSourceUrl, {
                                              ...SUBTITLE_BROWSER_HEADERS,
                                              ...(key ? { 'X-API-Key': key, 'Authorization': `Bearer ${key}` } : {})
                                            }, 25000, 1);
                                          } else if (guideProvider === 'subsource' && guideSubtitleId) {
                                            const key = String(sourceConfig.subsourceKey || '');
                                            if (key) {
                                              const r = await axios.get(`${SUBSOURCE_API}/subtitles/${encodeURIComponent(guideSubtitleId)}/download`,
                                                { headers: getSubsourceHeaders(key), responseType: 'arraybuffer', timeout: 25000, maxRedirects: 5 });
                                              sdhEvidenceSrt = extractSubtitleText(Buffer.from(r.data));
                                            }
                                          }
                                          if (sdhEvidenceSrt) console.log(`🦻 [SDH Guide] loaded ${sdhEvidenceSrt.length.toLocaleString()} chars`);
                                        } catch (err) {
                                          console.warn('[SDH Guide] fetch failed; fallback:', err.response?.status || '', err.message);
                                          sdhEvidenceSrt = '';
                                        }
                                      }

                                      const selectedSdhTags = buildSdhEvidenceSample(originalSrt, 3000)
                                        .split('\n').filter(x => x.startsWith('[SPEAKER TAG]')).length;
                                      if (selectedSdhTags >= 3) {
                                        originalSrt = cleanSelectedSdhForTranslation(originalSrt);
                                        console.log('🧹 [SDH Cleanup] selected source cleaned before translation.');
                                      }


                                      let movieContext = 'Phim điện ảnh/truyền hình tổng quát. Chưa có metadata từ Cinemeta.';
                                      let cinemetaMeta = null;
                                      let castGenderEvidence = '';
                                      let castAgeEvidence = '';
                                      if (imdbId) {
                                        try {
                                          const metaRes = await axios.get(`https://v3-cinemeta.strem.io/meta/${type === 'series' ? 'series' : 'movie'}/${imdbId}.json`, { timeout: 5000 });
                                          const meta = metaRes.data?.meta;
                                          cinemetaMeta = meta || null;
                                          if (meta) {
                                            movieContext =
                                              `Tên phim: ${meta.name || ''}` +
                                      ...
                                      // Build structured cast evidence from the Cinemeta object already fetched above.
                                      // If Cinemeta does not expose structured cast/gender/age, fall back safely to SDH/dialogue.
                                      try {
                                        const rawCastCandidates = [
                                          cinemetaMeta?.cast,
                                          cinemetaMeta?.credits?.cast,
                                          cinemetaMeta?.meta?.cast,
                                          cinemetaMeta?.meta?.credits?.cast
                                        ];
                                        const rawCast = rawCastCandidates.find(Array.isArray) || [];

                                        const castItems = buildCastGenderEvidence(rawCast);
                                        castGenderEvidence = formatCastGenderEvidence(castItems, 6000);

                                        const ageItems = buildCastAgeEvidence(rawCast);
                                        castAgeEvidence = formatCastAgeEvidence(ageItems, 5000);

                                        if (castItems.length) {
                                          console.log(`👥 [Character Gender] ${castItems.length} cast/character mappings available`);
                                        } else {
                                          console.log('👥 [Character Gender] no structured gender metadata; SDH/dialogue fallback');
                                        }
                                        if (ageItems.length) {
                                          console.log(`🎂 [Character Age] ${ageItems.length} character/cast age-group mappings available`);
                                        } else {
                                          console.log('🎂 [Character Age] no structured age metadata; relationship/dialogue fallback');
                                        }
                                      } catch (e) {
                                        console.warn('[Character Metadata] extraction skipped:', e.message);
                                        castGenderEvidence = '';
                                        castAgeEvidence = '';
                                      }

                                  const subtitleSample = buildSubtitleContextSample(originalSrt, 9000);
                                      const sdhSample = buildSdhEvidenceSample(sdhEvidenceSrt, 9000);
                                      ...
                                      const protectedTerms = extractProtectedTermsFromGuide(relationshipGuide);
              const protectedTermsPrompt = formatProtectedTermsForPrompt(protectedTerms, 6000);
              if (protectedTerms.length) {
                console.log(`🔒 [Protected Terms] ${protectedTerms.length} names/terms locked`);
              }
              ...
              const contextGuide = `
                                  ...
                                  SDH speaker/context evidence:
                                  ${sdhSample || 'Không có SDH evidence.'}

                                  Cast/character gender evidence:
                                  ${castGenderEvidence || 'Không có structured cast/gender evidence.'}

                                  Character/cast age evidence:
                                  ${castAgeEvidence || 'Không có structured cast/age evidence.'}

                                  PROTECTED NAMES/TERMS — GIỮ NGUYÊN 100%, KHÔNG DỊCH:
          ${protectedTermsPrompt || 'Không có protected term được khóa.'}
          ...
          QUY TẮC SỬ DỤNG BẢNG:
                                  ...
                                  - Gender trong Guide/Cast evidence dùng để tránh đảo vai người nói/người nghe; không tự suy ra hệ đại từ chỉ từ gender.
                          - Khi quyết định đại từ, xét theo thứ tự: quan hệ gia đình/chức danh/vai vế rõ -> relative_age -> gender -> mức thân mật/register.
                          - Nếu Guide khóa ông/cháu, bà/cháu, bác/cháu, chú/cháu, cô/cháu... thì PHẢI giữ đúng SELF/ADDRESS theo từng hướng.
                          - Không hạ một nhân vật elderly/much_older đã có evidence rõ xuống anh/em hoặc cậu/tớ chỉ vì một câu thoại riêng lẻ.
                          - Tuổi diễn viên chỉ là evidence phụ; không biến tuổi diễn viên thành tuổi chính xác của nhân vật.
                              - Không được đổi một character đã xác định male thành female hoặc ngược lại giữa các chunk nếu không có bằng chứng metadata đáng tin cậy hơn.
                              - PROTECTED NAMES/TERMS là immutable token: phải giữ nguyên chính tả và nội dung 100%; không dịch nghĩa, Việt hóa, diễn giải, rút gọn hoặc thay bằng từ gần nghĩa.
          - Nếu một protected term xuất hiện trong source cue thì output cue phải giữ đúng term đó. Ví dụ Malivore -> Malivore, tuyệt đối không đổi thành "thầy đồng" hay nghĩa suy diễn khác.
          - SELF = cách người nói tự xưng; ADDRESS = cách người nói gọi người nghe. Không đảo SELF/ADDRESS.
                                  - Nếu Guide có A->B hoặc B->A {self,address}, phải ưu tiên đúng chiều đó.
                                  - Không tự đổi anh/em, cậu/tớ, tôi/cô chỉ vì một câu đứng riêng lẻ.
                                  - Chỉ đổi hệ xưng hô khi có bằng chứng rõ về thay đổi quan hệ/vai vế trong chính tập.
        ...
        - Mọi mục trong PROTECTED NAMES/TERMS là BẤT BIẾN: copy nguyên văn từ source sang bản dịch, không dịch nghĩa hoặc Việt hóa.
        - Nếu không chắc một protected term có nghĩa gì, vẫn giữ nguyên; không được tự thay bằng từ tiếng Việt.
